  ...
  getRedirectUrl(options: { proxyProjectIdOverride?: string; urlPath?: string }): string {
    ...
    if (!legacyExpoProjectId) {
      ...
      if (Constants.manifest2) {
        nextSteps =
          ' The AuthSession proxy has been deprecated for modern applications. Prefer useAuthRequest (with the useProxy option set to false) in combination with an Expo Development Client build of your application.' +
          ' To continue using the AuthSession proxy during the transition, specify the project ID using the overrideProjectId option.';
      }
